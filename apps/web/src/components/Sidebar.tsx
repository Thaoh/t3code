...
export default function Sidebar() {
  ...
  const threadParkingNotesEnabled = useClientSettings((s) => s.threadParkingNotes);
  ...
  // Settled threads stay in the live shell stream (settled ≠ archived).
  // Archived threads remain hidden here — archive keeps its original
  ...
  const projectParkingUnsupported = useCallback(
    (environmentId: EnvironmentId) => {
      if (!threadParkingNotesEnabled) return false;
      const config = serverConfigs.get(environmentId);
      return config !== undefined && !serverConfigSupportsParkingNotes(config);
    },
    [serverConfigs, threadParkingNotesEnabled],
  );
                ...
                >
                  ...
                  >
                    ...
                    <span className="flex min-w-0 flex-1 items-center gap-2">
                      ...
                      {scopedProjectGroup &&
                      projectParkingUnsupported(scopedProjectGroup.environmentId) ? (
                        <Tooltip>
                          <TooltipTrigger
                            render={
                              <span
                          ...
                          <TooltipPopup side="top">
                            Parking notes for this project are stored on this device — the server
                            predates synced notes. They sync up automatically once the server is
                            updated.
                          </TooltipPopup>
                        </Tooltip>
                      ) : null}
                      ...
                      {(item: (typeof projectScopeItems)[number]) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {project && projectParkingUnsupported(project.environmentId) ? (
                              <Tooltip>
                                <TooltipTrigger
                                  render={
                                    <span
                                ...
                                <TooltipPopup side="top">
                                  Parking notes for this project are stored on this device — the
                                  server predates synced notes. They sync up automatically once the
                                  server is updated.
                                </TooltipPopup>
                              </Tooltip>
                            ) : null}
